...
def patch_rand_drop(args, x, x_rep=None, max_drop=0.3, max_block_sz=0.25, tolr=0.05):
    ...
    while total_pix < n_drop_pix:
        ...
        if x_rep is None:
            ...
            if torch.max(x) - torch.min(x) > 0:
                x_uninitialized = x_uninitialized * (torch.max(x) - torch.min(x)) + torch.min(x)
...
def aug_rand(samples, args=args):
